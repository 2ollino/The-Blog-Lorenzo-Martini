    ...
    <div class="container p-4 ">
        <div class="row justify-content-center align-items-center">
            <div class="col-12 col-md-8 card p-4">
                <form class="row g-3" action="{{ route('article.update' , $article) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="category" id="category" class="form-control">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{$category->id}}" @if($article->category_id == $category->id) selected @endif class="text-capitalize">{{$category->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label  class="form-label">Tags</label>
                        <input type="text" class="form-control" name="tags" value="{{ $article->tags->implode('name', ',') }}">
                    ...
                    <div class="col-12">
                        ...
                        <textarea type="text" class="form-control" cols="30" rows="7" name="body">{{ $article->body }}</textarea>
                        @error('body')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-12 text-center">
                        ...
                        <a href="{{ route('home') }}" class="text-decoration-none">Torna alla home</a>
                    </div>
                </form>
            </div>
        </div>
